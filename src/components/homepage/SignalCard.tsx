...
import {
    TrendingUp, TrendingDown, Target, ShieldAlert, ShieldCheck, Ban,
...
export const SignalCard: React.FC<{ signal: Signal }> = ({signal}) => {
    ...
    // Mutación para registrar en Bitácora
    ...
    const handleTradeClick = async () => {
        ...
        try {
            ...
            // Registrar en Base de Datos (Si hay riesgo configurado)
            if (calculations.riskAmount > 0) {
                logTradeMutation.mutate();
            }

            // Abrir Metatrader 5 desde el navegador
    ...
    return (
        <div
            className={`relative rounded-xl border ${borderColor} ${bgColor} p-4 mb-3 transition-all duration-300 group`}>

            {/* Encabezado */}
            <div className="flex justify-between items-start mb-3">
                <div className="flex items-center gap-2">
                    <div
                        className={`p-2 rounded-lg ${isBuy ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'}`}>
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2">
                            <span
                                className={`text-xs font-bold px-2 py-0.5 rounded ${isBuy ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'}`}>
                                {signal.signal_type}
                            </span>
            ...
            {/* Grid de Datos */}
            <div
                className="grid grid-cols-3 divide-x divide-gray-700/50 mt-2 bg-gray-900/40 rounded-lg border border-gray-700/30 overflow-hidden">
                <div
                    onClick={() => {
                        navigator.clipboard.writeText(signal.take_profit.toString());
                        toast.success('TP Copiado');
                    }}
                    className="flex flex-col items-center p-2 hover:bg-white/5 cursor-pointer active:bg-white/10"
                >
                    <span className="text-[10px] text-gray-400 font-bold mb-1 flex items-center gap-1"><Target
                        size={10}/> TP</span>
                    <span
                        className="text-white font-mono font-bold text-lg leading-none">{Math.round(signal.take_profit)}</span>
                ...
                <div
                    onClick={() => {
                        navigator.clipboard.writeText(signal.stop_loss.toString());
                        toast.success('SL Copiado');
                    }}
                    className="flex flex-col items-center p-2 hover:bg-white/5 cursor-pointer active:bg-white/10"
                >
                    <span className="text-[10px] text-gray-400 font-bold mb-1 flex items-center gap-1"><ShieldAlert
                        size={10}/> SL</span>
                    <span
                        className="text-white font-mono font-bold text-lg leading-none">{Math.round(signal.stop_loss)}</span>
                ...
                <div className="flex flex-col items-center justify-center p-2">
                    <span className="text-[10px] text-gray-400 font-bold mb-1 flex items-center gap-1"><Layers
                        size={10}/> Lotaje</span>
